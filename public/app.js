...
const app = {
  ...
  updateUI() {
    ...
    if (this.user) {
      btns.innerHTML = `
        <button class="btn btn-lang" id="langBtn">${this.lang === 'en' ? 'RU' : 'EN'}</button>
        <span style="color: #4ade80; margin-right: 15px;">${this.escapeHtml(this.user.username)}${this.user.isAdmin ? ' 👑' : ''}</span>
        <button class="btn" id="logoutBtn"><span data-i18n="logout">${this.t('logout')}</span></button>
      `;
      ...
      if (createThreadBtn) {
        ...
        const newBtn = createThreadBtn.cloneNode(true);
        createThreadBtn.parentNode.replaceChild(newBtn, createThreadBtn);
        newBtn.addEventListener('click', () => this.showModal('createThreadModal'));
      }
  ...
  renderPublic() {
    ...
    container.innerHTML = threads.map((t, i) => `
      <div class="thread-item" data-thread-id="${t.id}" style="animation-delay: ${i * 0.05}s; cursor: pointer;">
    ...
    container.querySelectorAll('.thread-item').forEach(item => {
      item.addEventListener('click', () => {
        const id = parseInt(item.getAttribute('data-thread-id'));
        this.viewThread(id);
      });
    });
  },
  ...
  async viewThread(id) {
    try {
      ...
      // Добавляем обработчик для формы ответа
      if (this.user) {
        const replyForm = document.getElementById('replyForm');
        if (replyForm) {
          replyForm.addEventListener('submit', (e) => this.addReply(e, id));
        }
      }
    } catch (err) {
      this.showToast(err.message, 'error');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const langBtn = document.getElementById('langBtn');
  if (langBtn) langBtn.addEventListener('click', () => app.toggleLang());
  
  const loginBtns = document.querySelectorAll('[onclick*="showModal(\'loginModal\')"]');
  ...
  navItems.forEach((item, idx) => {
    const sections = ['public', 'private', 'admin'];
    item.addEventListener('click', () => app.showSection(sections[idx]));
  });
  
  const createThreadBtn = document.getElementById('createThreadBtn');
  if (createThreadBtn) {
    createThreadBtn.addEventListener('click', () => app.showModal('createThreadModal'));
  }
  
  const backBtn = document.querySelector('.back-btn');
  if (backBtn) backBtn.addEventListener('click', () => app.backToList());
  
  const generateKeysBtn = document.querySelector('[onclick*="generateKeys"]');
  if (generateKeysBtn) generateKeysBtn.addEventListener('click', () => app.generateKeys());
  
  // Формы
  const loginForm = document.querySelector('#loginModal form');
  if (loginForm) loginForm.addEventListener('submit', (e) => app.login(e));
  
  const registerForm = document.querySelector('#registerModal form');
  if (registerForm) registerForm.addEventListener('submit', (e) => app.register(e));
  
  const threadForm = document.querySelector('#createThreadModal form');
  if (threadForm) threadForm.addEventListener('submit', (e) => app.createThread(e));
  
  // Ссылки переключения между модалками
  document.querySelectorAll('[onclick*="registerModal"]').forEach(link => {
  ...
  document.querySelectorAll('[onclick*="loginModal"][onclick*="registerModal"]').forEach(link => {
    link.addEventListener('click', (e) => {
      e.preventDefault();
      app.showModal('loginModal');
